...
public class SpeakersCallRepositoryImpl implements SpeakersCallRepository {
    ...
    public Observable<SpeakersCall> getSpeakersCall(long eventId, boolean reload) {
        Observable<SpeakersCall> diskObservable =
                Observable.defer(
                        () ->
                                repository.getItems(
                                        SpeakersCall.class,
                                        SpeakersCall_Table.event_id.eq(eventId)));

        Observable<SpeakersCall> networkObservable =
                Observable.defer(
                        () ->
                                speakersCallApi
                                        .getSpeakersCall(eventId)
                                        .doOnNext(
                                                speakersCall ->
                                                        repository
                                                                .save(
        ...
        return repository
                .observableOf(SpeakersCall.class)
                .reload(reload)
        ...
        return speakersCallApi
                ...
                .doOnNext(
                        created -> {
                            ...
                            repository.save(SpeakersCall.class, created).subscribe();
                        })
