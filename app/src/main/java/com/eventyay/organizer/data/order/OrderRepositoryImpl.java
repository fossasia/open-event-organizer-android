...
public class OrderRepositoryImpl implements OrderRepository {
    ...
    public Observable<Order> getOrders(long eventId, boolean reload) {
        Observable<Order> diskObservable =
                Observable.defer(
                        () -> repository.getItems(Order.class, Order_Table.event_id.eq(eventId)));

        Observable<Order> networkObservable =
                Observable.defer(
                        () ->
                ...
                Observable.defer(
                        () ->
                                repository
                                        .getItems(
                                                Order.class,
                                                Order_Table.identifier.eq(orderIdentifier))
                                        .take(1));
        ...
        return orderApi.postOrder(order)
                .doOnNext(
                        created -> {
                            ...
                            repository.save(Order.class, created).subscribe();
                        })
    ...
    public Observable<OrderStatistics> getOrderStatisticsForEvent(long eventId, boolean reload) {
        Observable<OrderStatistics> diskObservable =
                Observable.defer(
                        () ->
                                repository
                                        .getItems(
                                                OrderStatistics.class,
                                                OrderStatistics_Table.event_id.eq(eventId))
                                        .take(1));

        Observable<OrderStatistics> networkObservable =
                Observable.defer(
                        () ->
    ...
    public Completable sendReceipt(OrderReceiptRequest orderReceiptRequest) {
        ...
        return orderApi.sendReceiptEmail(orderReceiptRequest)
