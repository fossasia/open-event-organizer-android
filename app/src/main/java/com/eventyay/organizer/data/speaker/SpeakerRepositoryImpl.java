...
public class SpeakerRepositoryImpl implements SpeakerRepository {
    ...
    public Observable<Speaker> getSpeakers(long eventId, boolean reload) {
        Observable<Speaker> diskObservable =
                Observable.defer(
                        () ->
                                repository.getItems(
                                        Speaker.class, Speaker_Table.event_id.eq(eventId)));

        Observable<Speaker> networkObservable =
                Observable.defer(
                        () ->
                                speakerApi
                                        .getSpeakers(eventId)
    ...
    public Observable<Speaker> getSpeaker(long speakerId, boolean reload) {
        Observable<Speaker> diskObservable =
                Observable.defer(
                        () ->
                                repository
                                        .getItems(Speaker.class, Speaker_Table.id.eq(speakerId))
                                        .take(1));
