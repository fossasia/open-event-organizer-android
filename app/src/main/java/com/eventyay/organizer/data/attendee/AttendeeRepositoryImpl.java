...
public class AttendeeRepositoryImpl implements AttendeeRepository {
    ...
    public Observable<Attendee> getAttendee(long attendeeId, boolean reload) {
        Observable<Attendee> diskObservable =
                Observable.defer(
                        () ->
                                repository
                                        .getItems(Attendee.class, Attendee_Table.id.eq(attendeeId))
                                        .take(1));

        // There is no use case where we'll need to load single attendee from network
        return repository
                .observableOf(Attendee.class)
                .reload(reload)
    ...
    public Observable<Attendee> getAttendees(long eventId, boolean reload) {
        Observable<Attendee> diskObservable =
                Observable.defer(
                        () ->
                                repository.getItems(
                                        Attendee.class, Attendee_Table.event_id.eq(eventId)));

        Observable<Attendee> networkObservable =
                Observable.defer(
                        () ->
                                attendeeApi
    ...
    @NonNull
    @Override
    public Observable<Attendee> getAttendeesPageWise(
            long eventId, long pageNumber, boolean reload) {
        Observable<Attendee> diskObservable =
                Observable.defer(
                        () ->
                                repository.getItems(
        ...
        Observable<Attendee> networkObservable =
                Observable.defer(
                        () ->
                                attendeeApi
                                        .getAttendeesPageWise(eventId, pageNumber)
                                        .doOnNext(
                                                attendees ->
                                                        repository
    ...
    @Override
    public Observable<Attendee> getAttendeesUnderOrder(
            String orderIdentifier, long orderId, boolean reload) {
        Observable<Attendee> diskObservable =
                Observable.defer(
                        () ->
                                repository.getItems(
                                        Attendee.class, Attendee_Table.order_id.eq(orderId)));

        Observable<Attendee> networkObservable =
                Observable.defer(
                        () ->
                                attendeeApi
                                        .getAttendeesUnderOrder(orderIdentifier)
                                        .doOnNext(
                                                attendees ->
                                                        repository
                                                                .syncSave(
                        ...
                        completableObserver -> {
                            ...
                            if (!repository.isConnected())
                                completableObserver.onError(
                                        new Exception("No network present. Added to job queue"));
                        })
                .subscribeOn(Schedulers.io());
    }
        ...
        return Observable.just(transitAttendee)
                .flatMap(
                        attendee -> {
                            ...
                            attendee.setCheckinTimes(
                                    DateUtils.formatDateToIso(LocalDateTime.now()));

                            return attendeeApi.patchAttendee(attendee.getId(), attendee);
                        })
                .doOnNext(attendee -> repository.update(Attendee.class, attendee).subscribe())
    ...
    public Observable<Attendee> getPendingCheckIns() {
        return repository.getItems(Attendee.class, Attendee_Table.checking.eq(true));
    }
}
