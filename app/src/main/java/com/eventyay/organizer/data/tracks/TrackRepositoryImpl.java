...
public class TrackRepositoryImpl implements TrackRepository {
    ...
    public Observable<Track> getTracks(long eventId, boolean reload) {
        Observable<Track> diskObservable =
                Observable.defer(
                        () -> repository.getItems(Track.class, Track_Table.event_id.eq(eventId)));

        Observable<Track> networkObservable =
                Observable.defer(
                        () ->
    ...
    public Observable<Track> createTrack(Track track) {
        ...
        return trackApi.postTrack(track)
                .doOnNext(
                        created -> {
                            created.setEvent(track.getEvent());
                            repository.save(Track.class, created).subscribe();
                        })
    ...
    public Observable<Track> getTrack(long trackId, boolean reload) {
        Observable<Track> diskObservable =
                Observable.defer(
                        () -> repository.getItems(Track.class, Track_Table.id.eq(trackId)).take(1));
    ...
    public Observable<Track> updateTrack(Track track) {
        ...
        return trackApi.updateTrack(track.getId(), track)
                .doOnNext(updatedTrack -> repository.update(Track.class, updatedTrack).subscribe())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread());
    }
    ...
    public Completable deleteTrack(long id) {
        ...
        return trackApi.deleteTrack(id)
                .doOnComplete(
                        () -> repository.delete(Track.class, Track_Table.id.eq(id)).subscribe())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread());
    }
}
