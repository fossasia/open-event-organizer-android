...
public class OrgaApplication extends MultiDexApplication implements HasActivityInjector {
    ...
    public void onCreate() {
        ...
        if (CREATED.getAndSet(true)) return;
        ...
        if (BuildConfig.DEBUG) {
            ...
            StrictMode.ThreadPolicy.Builder policyBuilder =
                    new StrictMode.ThreadPolicy.Builder().detectAll().penaltyDeath();

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                /**
                 * https://medium.com/@elye.project/walk-through-hell-with-android-strictmode-7e8605168032
                 * "If you really like penaltyDeath(). Perhaps we could turn that permitDiskReads()
                 * by default. Most of the detected violation from other sources that really need
                 * suppression are Disk Reading error."
                 */
                policyBuilder.permitDiskReads();
            }
    ...
    private static class ReleaseLogTree extends Timber.Tree {
        ...
        protected void log(int priority, String tag, @NonNull String message, Throwable throwable) {
            ...
            if (priority == Log.INFO) {
                ...
                Sentry.getContext()
                        .recordBreadcrumb(new BreadcrumbBuilder().setMessage(message).build());
            }

            if (priority == Log.ERROR) {
                if (throwable == null) Sentry.capture(message);
                else Sentry.capture(throwable);
