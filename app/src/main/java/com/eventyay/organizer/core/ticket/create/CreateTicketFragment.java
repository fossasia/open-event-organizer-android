    ...
    public View onCreateView(
        ...
        binding =
                DataBindingUtil.inflate(
                        localInflater, R.layout.ticket_create_layout, container, false);
        binding.form.name.requestFocus();
        ViewUtils.showKeyboard(getContext());
        createTicketViewModel =
                ViewModelProviders.of(this, viewModelFactory).get(CreateTicketViewModel.class);
        validator = new Validator(binding.form);

        binding.submit.setOnClickListener(
                view -> {
                    if (validator.validate()) createTicketViewModel.createTicket();
                });
