...
public class OrderDetailViewModel extends ViewModel {
    ...
    public LiveData<Order> getOrder(String identifier, long eventId, boolean reload) {
        if (orderLiveData.getValue() != null && !reload) return orderLiveData;

        compositeDisposable.add(
                orderRepository
                        .getOrder(identifier, reload)
                        .compose(dispose(compositeDisposable))
                        .doOnSubscribe(disposable -> progress.setValue(true))
                        .doFinally(() -> progress.setValue(false))
                        .subscribe(
                                orderLiveData::setValue,
                                throwable ->
                                        error.setValue(
    ...
    public void getEvent(long eventId) {
        compositeDisposable.add(
                eventRepository
                        ...
                        .doFinally(() -> progress.setValue(false))
                        .subscribe(
                                event -> orderLiveData.getValue().setEvent(event),
                                throwable ->
                                        error.setValue(
                                                ErrorUtils.getMessage(throwable).toString())));
    }

    public LiveData<List<Attendee>> getAttendeesUnderOrder(
            String orderIdentifier, long orderId, boolean reload) {
        if (attendeesLiveData.getValue() != null && !reload) return attendeesLiveData;

        compositeDisposable.add(
                attendeeRepository
                        .getAttendeesUnderOrder(orderIdentifier, orderId, reload)
                        .compose(dispose(compositeDisposable))
                        .doOnSubscribe(disposable -> progress.setValue(true))
                        .doFinally(() -> progress.setValue(false))
                        .toList()
                        .subscribe(
                                attendeesLiveData::setValue,
                                throwable ->
                                        error.setValue(
                                                ErrorUtils.getMessage(throwable).toString())));
    ...
    public LiveData<List<Ticket>> getTicketsUnderOrder(
            String orderIdentifier, long orderId, boolean reload) {
        if (ticketsLiveData.getValue() != null && !reload) return ticketsLiveData;

        compositeDisposable.add(
                ticketRepository
                        .getTicketsUnderOrder(orderIdentifier, orderId, reload)
                        .compose(dispose(compositeDisposable))
                        .doOnSubscribe(disposable -> progress.setValue(true))
                        .doFinally(() -> progress.setValue(false))
                        .toList()
    ...
    public void toggleCheckIn(int index) {
        ...
        attendeesLiveData.getValue().get(index).isCheckedIn =
                !attendeesLiveData.getValue().get(index).isCheckedIn;

        compositeDisposable.add(
                attendeeRepository
                        .scheduleToggle(attendeesLiveData.getValue().get(index))
                        .subscribe(
                                () -> {
                                    // Nothing here
                                },
    ...
    public void sendReceipt(String orderIdentifier) {
        ...
        compositeDisposable.add(
                orderRepository
                        .sendReceipt(orderReceipt)
                        .doOnSubscribe(disposable -> progress.setValue(true))
                        .doFinally(() -> progress.setValue(false))
                        .subscribe(
                                () -> success.setValue("Email Sent!"),
                                throwable ->
                                        error.setValue(
                                                ErrorUtils.getMessage(throwable).toString())));
    }
