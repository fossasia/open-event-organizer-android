...
public class OrganizerDetailViewModel extends ViewModel {
    ...
    private final SubmitEmailVerificationToken submitEmailVerificationToken =
            new SubmitEmailVerificationToken();
    ...
    public void resendVerificationMail() {
        compositeDisposable.add(
                authService
                        ...
                        .doFinally(() -> progress.setValue(false))
                        .subscribe(
                                resendMailResponse -> success.setValue("Verification Mail Resent"),
                                throwable -> {
                                    error.setValue(
    ...
    public void verifyMail() {
        compositeDisposable.add(
                authService
                        .verifyMail(submitEmailVerificationToken)
                        .doOnSubscribe(disposable -> progress.setValue(true))
                        .doFinally(() -> progress.setValue(false))
                        .subscribe(
                                () -> success.setValue("Mail Verified"),
                                throwable -> {
                                    error.setValue(
                                            ErrorUtils.getErrorDetails(throwable).toString());
                                    Timber.e(
