...
class ToolbarColorChanger {
    ...
    Observable<Integer> observeColor(
        ...
        onOffsetChangedListener =
                (appBarLayout, verticalOffset) ->
                        offsetChanger.onNext(
                                (collapsingToolbar.getHeight() + verticalOffset)
                                        < (2 * ViewCompat.getMinimumHeight(collapsingToolbar)));
        appBar.addOnOffsetChangedListener(onOffsetChangedListener);

        return offsetChanger
                .distinctUntilChanged()
                .map(
                        collapsed -> {
                            if (collapsed) return resources.getColor(android.R.color.black);
                            else return resources.getColor(android.R.color.white);
                        });
    }
}
