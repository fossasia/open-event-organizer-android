...
public class AutoCheckInFragment extends BaseFragment implements AutoCheckInView {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.auto_check_in_fragment, container, false);

        autoCheckInViewModel = ViewModelProviders.of(getActivity(), viewModelFactory).get(AutoCheckInViewModel.class);

        autoCheckInViewModel.getProgress().observe(this, this::showProgress);
        autoCheckInViewModel.getError().observe(this, this::showError);
        autoCheckInViewModel.getTickets().observe(this, (newTickets) -> {
            ticketsAdapter.setTickets(newTickets);
            autoCheckInAll();
        });
        autoCheckInViewModel.getTicketUpdatedAction().observe(this, (aVoid) -> {
            autoCheckInAll();
        });

        autoCheckInViewModel.loadTickets(eventId);

        binding.autoCheckInAll.setOnClickListener(v -> {
            autoCheckInAll(!binding.autoCheckInAllCheckbox.isChecked());
        });
        binding.autoCheckInAllCheckbox.setOnClickListener(v -> {
    ...
    private void setupRecyclerView() {
        ticketsAdapter = new AutoCheckInAdapter(autoCheckInViewModel.getTickets().getValue(), autoCheckInViewModel::updateTicket);
