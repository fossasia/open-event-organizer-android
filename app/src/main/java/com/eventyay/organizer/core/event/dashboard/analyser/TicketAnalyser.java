...
public class TicketAnalyser {
    ...
    public TicketAnalyser(
            TicketRepository ticketRepository, AttendeeRepository attendeeRepository) {
    ...
    public void analyseTotalTickets(@NonNull Event event) {
        ticketRepository
                .getTicketsQuantity(event.getId())
                .doOnNext(
                        typeQuantity -> {
    ...
    public void analyseSoldTickets(@NonNull Event event, @NonNull List<Attendee> attendees) {
        ...
        ticketRepository
                .getSoldTicketsQuantity(event.getId())
                .doOnNext(
                        typeQuantity -> {
                            switch (typeQuantity.getType()) {
                                case "free":
                                ...
                                case "paid":
                                    ...
                                    break;
                                case "donation":
                                    event.analytics.soldDonationTickets.set(
                                            typeQuantity.getQuantity());
                                    break;
                                default:
                                    // Nothing
                            }
        ...
        ticketRepository
                .getTotalSale(event.getId())
                .subscribe(event.analytics.totalSale::set, Logger::logError);

        attendeeRepository
                .getCheckedInAttendees(event.getId())
