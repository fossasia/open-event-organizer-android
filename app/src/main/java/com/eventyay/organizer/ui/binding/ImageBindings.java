...
public final class ImageBindings {
    ...
    private static void setGlideImage(
            ImageView imageView,
            String url,
            Drawable drawable,
            Consumer<GlideRequest<Drawable>> consumer) {
        if (TextUtils.isEmpty(url)) {
            if (drawable != null) imageView.setImageDrawable(drawable);
            return;
        }
        GlideRequest<Drawable> request = GlideApp.with(imageView.getContext()).load(Uri.parse(url));

        if (drawable != null) {
            request.placeholder(drawable).error(drawable);
        }
        request.centerCrop().transition(withCrossFade());
    ...
    @BindingAdapter(
            value = {"circleImageUrl", "placeholder"},
            requireAll = false)
    ...
    @BindingAdapter(
            value = {"paletteImageUrl", "placeholder", "paletteKey", "paletteProfile"},
            requireAll = false)
    public static void bindImageWithPalette(
            ImageView imageView, String url, Drawable drawable, String key, int profile) {
        if (key == null)
            throw new IllegalArgumentException(
                    "A non null 'paletteKey' attribute must be provided for paletteImageUrl");

        GlidePalette<Drawable> glidePalette = GlidePalette.with(url).use(profile).crossfade(true);
