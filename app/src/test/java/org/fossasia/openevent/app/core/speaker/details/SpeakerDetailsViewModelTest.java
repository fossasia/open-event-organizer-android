...
public class SpeakerDetailsViewModelTest {
    ...
    private final long SPEAKER_ID = 1;
    ...
    public void shouldLoadSpeakerSuccessfully() {
        when(speakerRepository.getSpeaker(anyLong(), anyBoolean())).thenReturn(Observable.just(SPEAKER));
    ...
    public void shouldShowErrorOnLoadingUnsuccessfully() {
        when(speakerRepository.getSpeaker(anyLong(), anyBoolean())).thenReturn(Observable.error(new Throwable("Error")));
    ...
    public void shouldShowErrorMessageOnSwipeRefersh() {
        when(speakerRepository.getSpeaker(SPEAKER_ID, true)).thenReturn(Observable.error(Logger.TEST_ERROR));
