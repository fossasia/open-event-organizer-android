...
public class EventViewModelTest {
    ...
    @Mock Observer<Long> eventId;
    @Mock Observer<Void> singleEventAction;
    ...
    public void setUp() {
        eventViewModel =
                new EventViewModel(sharedPreferenceModel, bus, currencyUtils, eventRepository);
    }

    @Test
    public void shouldLoadSelectedEvent() {
        when(bus.getSelectedEvent()).thenReturn(Observable.just(EVENT));
        when(sharedPreferenceModel.getLong(EVENT_KEY, -1)).thenReturn(ID);

        InOrder inOrder =
                Mockito.inOrder(bus, sharedPreferenceModel, eventId, event, singleEventAction);
    ...
    public void shouldShowErrorInLoadingSelectedEvent() {
        when(bus.getSelectedEvent()).thenReturn(Observable.error(Logger.TEST_ERROR));
    ...
    public void shouldShowEventList() {
        when(bus.getSelectedEvent()).thenReturn(Observable.empty());
        when(sharedPreferenceModel.getLong(EVENT_KEY, -1)).thenReturn(-1L);
