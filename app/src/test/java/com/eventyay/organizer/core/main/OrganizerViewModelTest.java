...
public class OrganizerViewModelTest {
    ...
    @Mock private RxSharedPreferences sharedPreferences;
    @Mock private ContextManager contextManager;
    @Mock private Preference<Boolean> booleanPref;

    @Mock Observer<User> organizer;
    @Mock Observer<String> error;
    @Mock Observer<Void> singleEventObserver;
    ...
    public void setUp() {
        organizerViewModel =
                new OrganizerViewModel(
                        userRepository, authService, sharedPreferences, contextManager);
    }

    @Test
    public void shouldLoadOrganizerSuccessfully() {
        when(userRepository.getOrganizer(false)).thenReturn(Observable.just(ORGANIZER));
    ...
    public void shouldLogoutSuccessfully() {
        when(authService.logout()).thenReturn(Completable.complete());
    ...
    public void shouldShowLogoutError() {
        ...
        when(authService.logout()).thenReturn(Completable.error(Logger.TEST_ERROR));
