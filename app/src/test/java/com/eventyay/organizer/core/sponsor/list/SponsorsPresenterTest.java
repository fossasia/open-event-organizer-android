...
public class SponsorsPresenterTest {
    ...
    @Mock private SponsorsView sponsorsView;
    @Mock private SponsorRepository sponsorRepository;
    @Mock private DatabaseChangeListener<Sponsor> databaseChangeListener;

    private static final long ID = 42;

    private static final List<Sponsor> SPONSORS =
            Arrays.asList(
    ...
    public void shouldLoadSponsorsAutomatically() {
        when(sponsorRepository.getSponsors(anyLong(), anyBoolean()))
                .thenReturn(Observable.fromIterable(SPONSORS));
    ...
    public void shouldShowSponsorsAutomatically() {
        when(sponsorRepository.getSponsors(ID, false))
                .thenReturn(Observable.fromIterable(SPONSORS));
        when(databaseChangeListener.getNotifier()).thenReturn(PublishSubject.create());
    ...
    public void shouldShowEmptyViewOnNoSponsors() {
        ...
        verify(sponsorsView).showEmptyView(true);
    }
    ...
    public void shouldShowErrorMessageOnSwipeRefreshError() {
        when(sponsorRepository.getSponsors(ID, true))
    ...
    public void shouldNotDeleteUnselectedSponsors() {
        for (Long sponsorId : sponsorsPresenter.getSelectedSponsors().keySet()) {
            when(sponsorRepository.deleteSponsor(sponsorId))
