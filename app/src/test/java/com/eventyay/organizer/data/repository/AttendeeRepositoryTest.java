...
public class AttendeeRepositoryTest {
    ...
    private static final List<Attendee> ATTENDEES =
            Arrays.asList(new Attendee(), new Attendee(), new Attendee());

    @Before
    public void setUp() {
        when(repository.observableOf(Attendee.class))
    ...
    public void shouldLoadAttendeesFromCache() {
        ...
        attendeeObservable.toList().test().assertNoErrors().assertValue(ATTENDEES);
    ...
    public void shouldFetchAttendeesOnForceReload() {
        ...
        when(repository.syncSave(eq(Attendee.class), any(), any(), any()))
                .thenReturn(Completable.complete());
        when(repository.getItems(eq(Attendee.class), any(SQLOperator.class)))
                .thenReturn(Observable.fromIterable(ATTENDEES));

        // Force reload ensures no use of cache
        Observable<List<Attendee>> attendeeObservable =
                attendeeRepository.getAttendees(23, true).toList().toObservable();

        attendeeObservable.test().assertNoErrors().assertValue(ATTENDEES);
    ...
    public void shouldSendErrorOnNetworkDown() {
        ...
        attendeeRepository.getAttendees(43, false).test().assertErrorMessage(Constants.NO_NETWORK);

        attendeeRepository.getAttendees(43, true).test().assertErrorMessage(Constants.NO_NETWORK);

        attendeeRepository
                .toggleAttendeeCheckStatus(ATTENDEES.get(0))
                .test()
    ...
    public void shouldSaveToggledAttendeeCheck() {
        ...
        Observable<Attendee> attendeeObservable =
                attendeeRepository.toggleAttendeeCheckStatus(attendee);
